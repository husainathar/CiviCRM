...
<div class="crm-block crm-content-block crm-membership-view-form-block">
  ...
  {if $campaign}<tr><td class="label">{ts}Campaign{/ts}</td><td>{$campaign}</td></tr>{/if}
	...
	{if $membershipPeriodInfo}
        <div class="crm-accordion-wrapper">
              <div class="crm-accordion-header">{ts}Membership Period{/ts}</div>
              <div class="crm-accordion-body">{include file="CRM/Member/Form/MembershipPeriod.tpl" context="Search"}</div>
        </div>
    {/if}	
